...
import ttIcon from "@/assets/images/icon-twitter.svg"
import WhatsappIcon from "@/assets/images/whatsapp.svg"
import gitbubIcon from "@/assets/images/github.svg"
    ...
    <footer className="bg-[#2b2828] border-t border-white/30">
      <div className=" py-4 px-8 text-white min-w-[95%] md:max-w-[1400px] p-4 mx-auto">
           ...
           <a  href="/"><Image width={100} className="mx-auto md:mx-0 mb-4 md:m-0" src={logo} alt="cyberohn logo" /></a>
             <ul className="flex items-center gap-2 flex-col md:flex-row pb-8 mb-4 md:m-0 md:p-0 text-[#cac5d8]">
                {["About", "Contact", "Project Gallery"].map( (navItem, index) => <li className="cursor-pointer hover:font-bold transition-all w-[120px] text-center md:text-left" key={index}><a href={`/#${navItem.toLowerCase().replace(/\s+/g, "-")}` }>{navItem}</a></li>)}
              </ul>
